...
async def create_user(user_in: UserCreate, db: AsyncSession) -> User:
    """
    Создаёт нового пользователя, проверяя уникальность email.
    """
    ...
    user = User(
        ...
        role=user_in.role or "user",  # если роль приходит в UserCreate
    )
    db.add(user)
    await db.commit()
...
async def delete_unverified_users(db: AsyncSession) -> None:
    ...
    q = await db.execute(
        select(User).filter(User.is_verified == False, User.created_at < threshold)
    )
